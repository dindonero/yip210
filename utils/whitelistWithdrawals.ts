...
import { GOV, RESERVES, STETH, TIMELOCK, USDC, WHALE_ADDRESS, WHALE_ADDRESS_2 } from "../helper-hardhat-config";
...
export const whitelistWithdrawals = async (proposalAddress: string) => {
  ...
  const fundTx = await deployer.sendTransaction({to: WHALE_ADDRESS, value: ethers.utils.parseEther("100")})
  const fundTx2 = await deployer.sendTransaction({to: WHALE_ADDRESS_2, value: ethers.utils.parseEther("100")})
  await fundTx.wait(1)
  ...
  const calldatas = [ethers.utils.defaultAbiCoder.encode(["address[]", "uint256[]", "address[]"], [whos, amounts, tokens])]
  ...
  // Propose
  const proposeTx = await govWithSigner.propose(targets, values, signatures, calldatas, description)
  ...
  console.log("USDC allowance: ", await usdcContract.allowance(RESERVES, proposalAddress))
...
whitelistWithdrawals("0x0C04D9e9278EC5e4D424476D3Ebec70Cb5d648D1")
.then(() => {
  console.log("done")
})