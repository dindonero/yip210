...
import { AggregatorV3Interface, IERC20, ILido, ITimelock, IYamGovernorAlpha, YIP210 } from "../typechain-types";
import { GOV, RESERVES, STETH, TIMELOCK, USDC, WETH, WHALE_ADDRESS, WHALE_ADDRESS_2 } from "../helper-hardhat-config";
...
describe("YIP210", function () {
    ...
    before(async () => {
        ...
        priceFeedStETH = await ethers.getContractAt("AggregatorV3Interface", "0xCfE54B5cD566aB89272946F602D76Ea879CAb4a8")
        priceFeedUSDC = await ethers.getContractAt("AggregatorV3Interface", "0x8fFfFfd4AfB6115b954Bd326cbe7B4BA576818f6")
    })
    ...
    it("should rebalance when more diff than 70/30 ratio (sell usdc for steth)", async () => {
        ...
        const [target2, value2, signature2, calldata2] = await callExecute(YIP210.address)
        ...
        const stethValue = stethPrice.div(ethers.BigNumber.from(10).pow(8)).mul(finalStethBalanceReserves)
        const usdcValue = usdcPrice.mul(ethers.BigNumber.from(10).pow(4)).mul(finalUsdcBalanceReserves)
    ...
    it(("should not rebalance when less diff than 70/30 ratio"), async () => {
        ...
        expect(govWithSigner.execute(id)).to.be.revertedWith("Timelock::executeTransaction: Transaction execution reverted.")
    })
